...
const workspaceStatuses = new Set<ApplicationStatus>(["DRAFT","SAVED","PLANNING","PREPARING","WAITING_FOR_RECOMMENDATION","READY_TO_SUBMIT"]);
      ...
      {applications.map((application) => (
        ...
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs font-medium text-muted-foreground">
                  {workspaceStatuses.has(application.status) ? "Updated " : "Submitted "}
            ...
            {workspaceStatuses.has(application.status) ? <Link className="inline-flex min-h-10 items-center rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground" href={`/dashboard/student/applications/${application.id}`}>Continue workspace</Link> : null}
          </div>
