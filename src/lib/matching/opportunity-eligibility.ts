...
export function evaluateOpportunityEligibility({
  ...
  if (
    ...
    opportunity.deadline.getTime() < now.getTime()
  ) {
  ...
  if (opportunity.minimumAge == null && opportunity.maximumAge == null) {
    ...
    concerns.push(
      "Add your age in years to check the published age requirement.",
    );
  } else {
    if (
      opportunity.minimumAge != null &&
      student.ageYears < opportunity.minimumAge
    ) {
    ...
    if (
      opportunity.maximumAge != null &&
      student.ageYears > opportunity.maximumAge
    ) {
      blockingReasons.push(
  ...
  } else if (!parseGradeLevelCode(student.gradeYear)) {
    unknowns.push(
      "Your grade level could not be matched to a standard grade code.",
    );
  } else if (
    grades.some(
      (grade) =>
        parseGradeLevelCode(grade) === parseGradeLevelCode(student.gradeYear),
    )
  ) {
    confirmedMatches.push("Accepts your current grade level.");
  } else {
    blockingReasons.push(
      `Your grade level (${student.gradeYear}) is not in the published accepted grade levels (${grades
        .map((grade) => {
          const code = parseGradeLevelCode(grade);
          return code ? formatGradeLevelCode(code) : grade;
        })
        .join(", ")}).`,
    );
  }
  ...
  if (opportunity.geographicScope) {
    ...
    if (!studentLocation) {
      ...
      requiredLocation &&
      !overlaps([studentLocation], requiredLocation)
    ) {
      blockingReasons.push(`The listing is restricted to ${requiredLocation}.`);
    } else if (requiredLocation) {
      confirmedMatches.push(
        "Your profile matches the published geographic scope.",
      );
    } else {
      unknowns.push(
        "The geographic restriction is not specific enough to confirm.",
      );
    }
  }
  ...
  if (requiredCertifications.length > 0) {
    if (!student?.certifications?.length) {
      concerns.push(
        "Add certifications to check the published certification requirements.",
      );
    } else {
      ...
      if (missing.length > 0) {
        blockingReasons.push(
          `Missing required certification: ${missing.join(", ")}.`,
        );
      } else {
        confirmedMatches.push(
          "Meets the published certification requirements.",
        );
      }
    }
  }
  ...
  if (opportunity.scheduleRequirements) {
    unknowns.push(
      "Review the published schedule requirements before applying.",
    );
  }

  if (
    student &&
    overlaps(student.interestedSpecialties, opportunity.specialty)
  ) {
